...
function renderCatalog(products) {
    ...
    if (!grid) return;
    if (!products.length) {
        grid.innerHTML = '<p class="opacity-70 text-sm">������� ���� ����. �������� ������ � �������.</p>';
        return;
    }
        ...
        btn.addEventListener("click", async () => {
            ...
            if (inline.classList.contains("hidden")) {
                try {
                    ...
                    const reviews = await res.json();
                    if (reviews.length === 0) {
                        inline.innerHTML = '<p class="text-xs opacity-60">���� ��� �������</p>';
                    } else {
                ...
                } catch(e) { inline.innerHTML = '<p class="text-xs opacity-60">������ ��������</p>'; }
    ...
    grid.querySelectorAll(".review-add-btn").forEach(btn => {
        btn.addEventListener("click", () => {
            ...
            const formHtml = '<form class="review-submit-form space-y-2 mt-2" data-product-id="' + pid + '">' +
                ...
                '</select>' +
                '<textarea name="text" required placeholder="��� �����..." class="w-full bg-black/30 border border-white/20 rounded p-2 text-xs min-h-12"></textarea>' +
                '<button class="px-3 py-1 bg-brand-green rounded text-xs">���������</button>' +
                '</form>';
            ...
            inline.querySelector(".review-submit-form").addEventListener("submit", async (e) => {
                ...
                if (toggle && !toggle.textContent.includes("������")) toggle.click();
...
function injectShell(active) {
    ...
    document.body.insertAdjacentHTML("afterbegin", `
        ...
        <dialog id="cart-modal" class="glass-panel rounded-3xl p-8 w-[min(520px,92vw)] text-slate-100">
            ...
            <div id="cart-empty-msg" class="text-sm opacity-70 mb-4 hidden">������� �����</div>
            ...
            <div id="inquiry-wrap" class="hidden">
                <p class="text-sm opacity-70 mb-3">��� �������� ������ � �� �������� � ����:</p>
                <form id="inquiry-form" class="space-y-3">
                    <input required name="name" placeholder="���" class="w-full bg-black/30 border border-white/20 rounded-lg p-3 text-sm outline-none">
                    <input required name="contact" placeholder="������� ��� Telegram" class="w-full bg-black/30 border border-white/20 rounded-lg p-3 text-sm outline-none">
                    <textarea name="message" placeholder="����������� � ������" class="w-full bg-black/30 border border-white/20 rounded-lg p-3 text-sm outline-none min-h-24"></textarea>
                    <button class="w-full py-3 bg-brand-green hover:bg-brand-hover rounded-lg font-medium">��������� ������</button>
                </form>
            </div>
            <p id="inquiry-status" class="text-sm mt-3 opacity-80"></p>
            <button id="cart-close" class="mt-4 text-sm opacity-70 hover:opacity-100">�������</button>
        </dialog>
    `);
}
